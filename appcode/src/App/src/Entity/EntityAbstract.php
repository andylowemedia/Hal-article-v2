...
abstract class EntityAbstract implements \JsonSerializable
{
    ...
    public function __set($name, $value)
    {
        ...
        if (('mapper' === $name) || !method_exists($this, $method)) {
            throw new EntityException('Invalid page property :' . $method);
        }
        $this->$method($value);
    }
...
//    }
    ...
    public function toArray()
    {
        ...
        foreach ($this as $key => $property) {
            if ($property instanceof EntityAbstract || $property instanceof ResultSetAbstract) {
    ...
    public function toArraySql()
    {
        ...
        foreach ($this as $key => $property) {
            $newKey = strtolower(preg_replace('/([a-z])([A-Z])/', '$1_$2', $key));
            if ($property instanceof EntityAbstract) {
